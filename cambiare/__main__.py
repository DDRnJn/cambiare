import argparse
import cambiare.framework.MusicConverterFramework as mcf
import os


def main():
    parser = argparse.ArgumentParser(prog="cambiare", description="A lightweight single staff music format conversion program")
    ...
    output_module = __import__(f"cambiare.output_plugins.{output_plugin}.{output_plugin}",
                              fromlist=f"cambiare.output_plugins.{output_plugin}")
